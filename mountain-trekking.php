...
<head>
  ...
  <script>tailwind.config={theme:{extend:{colors:{brand:'#10b981',dark:'#0a0a0a'},fontFamily:{heading:['Playfair Display','Georgia','serif'],sans:['Inter','sans-serif'],nav:['Montserrat','sans-serif']}}}}</script>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .section-tag{display:inline-flex;align-items:center;gap:.45rem;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);border-radius:999px;padding:.28rem .9rem;font-family:'Montserrat',sans-serif;font-size:.6rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#34d399;margin-bottom:.85rem}
    ...
    #back-top{position:fixed;bottom:2rem;right:2rem;z-index:999;width:44px;height:44px;border-radius:50%;background:rgba(16,185,129,.15);border:1px solid rgba(16,185,129,.3);color:#10b981;display:flex;align-items:center;justify-content:center;cursor:pointer;opacity:0;transform:translateY(20px);transition:all .3s}
    ...
    #main-nav.scrolled,#main-nav.solid{background:rgba(10,10,10,.96) !important;backdrop-filter:blur(20px);border-bottom:1px solid rgba(16,185,129,.1)}
    /* Tour cards */
    ...
    .pkg-card:hover{border-color:rgba(16,185,129,.3);transform:translateY(-4px);box-shadow:0 20px 50px rgba(0,0,0,.6)}
    ...
    .route-card:hover{border-color:rgba(16,185,129,.25);background:rgba(17,17,17,.95);transform:translateY(-3px)}
    /* Why cards */
    ...
    .why-card:hover{border-color:rgba(16,185,129,.2);transform:translateY(-3px)}
  </style>
</head>
<body>
<div class="scroll-progress" id="scroll-progress" style="position:fixed;top:0;left:0;height:2px;background:linear-gradient(90deg,#10b981,#059669);z-index:9999;width:0;transition:width .1s"></div>

<?php require_once 'includes/public_navbar.php'; ?>
<?php require_once 'includes/booking-modal.php'; ?>

<!-- ══════════ HERO ══════════ -->
<section style="position:relative;overflow:hidden;padding-top:68px;background:#0a0a0a">
  <!-- Background image -->
  ...
  <div style="position:relative;z-index:10;max-width:1280px;margin:0 auto;padding:4rem 2rem;display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:center">
    ...
    <div>
      <nav style="display:flex;align-items:center;gap:.45rem;font-family:'Montserrat',sans-serif;font-size:.6rem;color:rgba(255,255,255,.35);margin-bottom:1.25rem">
        <a href="<?= url() ?>" style="color:inherit;text-decoration:none" onmouseover="this.style.color='#10b981'" onmouseout="this.style.color='rgba(255,255,255,.35)'">Home</a>
      ...
      <h1 style="font-family:'Playfair Display',serif;font-size:clamp(2rem,4vw,3.2rem);font-weight:700;color:#fff;line-height:1.15;margin-bottom:1rem" class="reveal">
      ...
      <div style="display:flex;gap:.75rem;flex-wrap:wrap" class="reveal">
        <a href="#packages" style="display:inline-flex;align-items:center;gap:.5rem;font-family:'Montserrat',sans-serif;font-weight:700;font-size:.78rem;text-transform:uppercase;letter-spacing:.08em;padding:.85rem 1.75rem;border-radius:10px;background:linear-gradient(135deg,#059669,#10b981);color:#fff;text-decoration:none;box-shadow:0 4px 16px rgba(16,185,129,.3)">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem" class="reveal">
      <?php foreach ([
        ['fa-mountain','5,895m','Uhuru Peak','Africa\'s highest summit','#10b981'],
        ['fa-route','6+','Major Routes','Machame, Lemosho, Rongai...','#fbbf24'],
        ['fa-chart-line','85%+','Summit Rate','Above-average success','#34d399'],
      ...
      <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:1.25rem">
        ...
        <div style="font-family:'Playfair Display',serif;font-size:1.5rem;font-weight:700;color:#fff;line-height:1"><?= $s[1] ?></div>
...
<section style="padding:5rem 2rem;background:rgba(255,255,255,.01);border-top:1px solid rgba(255,255,255,.05)">
  <div style="max-width:1280px;margin:0 auto">
    <div style="text-align:center;margin-bottom:3rem" class="reveal">
      ...
      <h2 style="font-family:'Playfair Display',serif;font-size:clamp(1.7rem,3vw,2.4rem);font-weight:700;color:#fff">
        Tanzania's Most Trusted <span class="hero-grad">Trek Operator</span>
      </h2>
    </div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.25rem">
      <?php foreach ([
        ['fa-certificate','#10b981','KINAPA Certified','All guides certified by Tanzania National Parks Authority.'],
...
<!-- ══════════ PACKAGES ══════════ -->
<section id="packages" style="padding:5rem 2rem;background:#0a0a0a">
  <div style="max-width:1280px;margin:0 auto">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:2.5rem;flex-wrap:wrap;gap:1rem" class="reveal">
      <div>
        ...
        <h2 style="font-family:'Playfair Display',serif;font-size:clamp(1.6rem,2.5vw,2rem);font-weight:700;color:#fff">
          Trekking & Adventure Tours
        </h2>
      </div>
      <a href="<?= url('tours?tour_type=Trekking') ?>" style="font-family:'Montserrat',sans-serif;font-size:.72rem;font-weight:600;color:#10b981;text-decoration:none;display:flex;align-items:center;gap:.35rem">
      ...
      <div class="pkg-card reveal" style="transition-delay:<?= $i*80 ?>ms">
        <div style="position:relative;height:220px;overflow:hidden">
          ...
          <div style="position:absolute;top:.85rem;right:.85rem;font-family:'Montserrat',sans-serif;font-size:.6rem;color:#fff;padding:.28rem .65rem;border-radius:999px;background:rgba(0,0,0,.55);backdrop-filter:blur(8px)">
            <i class="fas fa-clock" style="color:#10b981;font-size:.5rem"></i> <?= e($tour['duration']) ?>
          </div>
        ...
        <div style="padding:1.25rem;display:flex;flex-direction:column;gap:.6rem">
          ...
          <h3 style="font-family:'Playfair Display',serif;font-size:1.05rem;font-weight:700;color:#fff;line-height:1.3"><?= e($tour['name']) ?></h3>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.75rem;border-top:1px solid rgba(255,255,255,.06);margin-top:.25rem">
            <div>
              ...
              <div style="font-family:'Playfair Display',serif;font-size:1.35rem;font-weight:700;color:#10b981"><?= formatPrice($tour['price']) ?><span style="font-family:'Montserrat',sans-serif;font-size:.52rem;font-weight:400;color:rgba(255,255,255,.3)">/person</span></div>
            </div>
            <div style="display:flex;gap:.45rem">
              <a href="<?= url('tour/'.e($tour['slug'])) ?>" style="display:inline-flex;align-items:center;gap:.35rem;font-family:'Montserrat',sans-serif;font-weight:700;font-size:.68rem;text-transform:uppercase;letter-spacing:.06em;padding:.55rem 1rem;border-radius:9px;background:linear-gradient(135deg,#059669,#10b981);color:#fff;text-decoration:none">
    ...
    <div style="text-align:center;padding:4rem;background:rgba(255,255,255,.02);border-radius:16px;border:1px solid rgba(255,255,255,.06)">
      ...
      <a href="<?= url('contact') ?>" style="display:inline-flex;align-items:center;gap:.4rem;margin-top:1.25rem;font-family:'Montserrat',sans-serif;font-weight:700;font-size:.72rem;padding:.65rem 1.5rem;border-radius:10px;background:rgba(16,185,129,.15);border:1px solid rgba(16,185,129,.2);color:#10b981;text-decoration:none">
...
<section style="padding:5rem 2rem;background:rgba(255,255,255,.015);border-top:1px solid rgba(255,255,255,.05)">
  <div style="max-width:1280px;margin:0 auto">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem">
      <?php foreach ([
        ['Machame','fa-mountain','#10b981','6–7 days','Most Popular','High','Scenic "Whiskey Route" — best acclimatisation profile.'],
      ...
      <div class="route-card reveal">
        ...
        <h3 style="font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:700;color:#fff;margin-bottom:.35rem"><?= $r[0] ?> Route</h3>
...
<!-- ══════════ BEST TIME + INCLUDED ══════════ -->
<section style="padding:5rem 2rem;background:#0a0a0a;border-top:1px solid rgba(255,255,255,.05)">
  <div style="max-width:1280px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:1.5rem">
    ...
    <div style="background:rgba(17,17,17,.8);border:1px solid rgba(255,255,255,.07);border-radius:18px;padding:2rem" class="reveal">
      ...
      <h3 style="font-family:'Playfair Display',serif;font-size:1.3rem;font-weight:700;color:#fff;margin-bottom:1.5rem">When to Climb Kilimanjaro</h3>
      <?php foreach ([
        ['Jan – Mar','#10b981','Peak Season ⭐','Clear skies, excellent visibility. Best for photography.'],
        ['Apr – May','#f87171','Avoid — Long Rains','Wet & slippery trails. Not recommended.'],
        ['Jun – Oct','#10b981','Peak Season ⭐','Dry & clear. Most popular. Book well in advance.'],
    ...
    <div style="background:rgba(17,17,17,.8);border:1px solid rgba(255,255,255,.07);border-radius:18px;padding:2rem" class="reveal">
      ...
      <h3 style="font-family:'Playfair Display',serif;font-size:1.3rem;font-weight:700;color:#fff;margin-bottom:1.5rem">Everything You Need</h3>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem">
        ...
        <div style="display:flex;align-items:center;gap:.55rem;padding:.45rem .5rem;border-radius:8px;background:rgba(255,255,255,.03)">
          <i class="fas <?= $inc[0] ?>" style="color:#10b981;font-size:.6rem;flex-shrink:0;width:14px;text-align:center"></i>
...
<!-- ══════════ CTA ══════════ -->
<section style="padding:5rem 2rem;background:linear-gradient(135deg,rgba(16,185,129,.06),rgba(5,150,105,.04));border-top:1px solid rgba(16,185,129,.12)">
  <div style="max-width:640px;margin:0 auto;text-align:center" class="reveal">
    ...
    <h2 style="font-family:'Playfair Display',serif;font-size:clamp(1.8rem,3vw,2.5rem);font-weight:700;color:#fff;margin-bottom:.75rem">
    ...
    <div style="display:flex;gap:.75rem;justify-content:center;flex-wrap:wrap">
      <a href="<?= url('booking') ?>" style="display:inline-flex;align-items:center;gap:.5rem;font-family:'Montserrat',sans-serif;font-weight:700;font-size:.78rem;text-transform:uppercase;letter-spacing:.08em;padding:.85rem 2rem;border-radius:10px;background:linear-gradient(135deg,#059669,#10b981);color:#fff;text-decoration:none;box-shadow:0 4px 16px rgba(16,185,129,.3)">
