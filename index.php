...
$pageTitle       = 'Tanzania Safari & Kilimanjaro Tours | Jambo Masai Tours';
...
if (empty($testimonials)) {
    $testimonials = [
        ['customer_name'=>'Sarah M.','country'=>'United Kingdom','rating'=>5,'review'=>'An absolutely life-changing experience. Our guide Joseph knew every animal by name. The Serengeti at sunrise is something I will never forget.','photo'=>'','source'=>'google'],
        ['customer_name'=>'David K.','country'=>'United States', 'rating'=>5,'review'=>'Jambo Masai exceeded every expectation. The camp was luxurious, the food incredible, and the wildlife encounters were beyond anything I imagined.','photo'=>'','source'=>'tripadvisor'],
        ['customer_name'=>'Lena H.','country'=>'Germany',        'rating'=>5,'review'=>'From booking to farewell, everything was flawless. The team truly cares about giving you the best possible experience.','photo'=>'','source'=>'google'],
    ];
}

/* Review platform summary + write-review links, used by the homepage
   reviews section and the /reviews page — kept in one place so both
   stay in sync. */
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .rev-summary-card{background:rgba(255,255,255,.04);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:1.4rem 1.6rem;display:flex;align-items:center;gap:1rem;transition:all .35s cubic-bezier(.4,0,.2,1)}
    .rev-summary-card:hover{transform:translateY(-4px);border-color:rgba(160,94,34,.3);box-shadow:0 16px 40px rgba(0,0,0,.25)}
    .rev-platform-badge{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
    .testi-card{position:relative}
    .testi-card .glass-card{height:100%;transition:all .35s cubic-bezier(.4,0,.2,1);position:relative;overflow:hidden}
    .testi-card .glass-card:hover{transform:translateY(-5px);border-color:rgba(160,94,34,.28);box-shadow:0 20px 45px rgba(0,0,0,.3)}
...
<section class="py-20 px-4 lg:px-0" id="testimonials">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl mx-auto mb-12 reveal">
      <div class="rev-summary-card">
        ...
        <a href="<?= e($googleReviewUrl) ?>" target="_blank" rel="noopener"
           class="hidden sm:inline-flex items-center gap-1.5 font-nav text-[.68rem] font-semibold text-white/50 hover:text-white transition-colors whitespace-nowrap">
          Write a review <i class="fas fa-arrow-right text-[.6rem]"></i>
        </a>
        <?php endif; ?>
      </div>
      <div class="rev-summary-card">
          ...
          <div class="glass-card p-6 h-full flex flex-col">
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="text-white/40 text-xs"><?= e($t['country']) ?><?= !empty($t['tour_name']) ? ' · ' . e($t['tour_name']) : '' ?></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <!-- CTAs: see all reviews + leave a review -->
    <div class="flex flex-wrap items-center justify-center gap-3 mt-12 reveal">
      <a href="<?= url('reviews') ?>" class="btn-outline">
        <i class="fas fa-comments text-xs"></i> See All Reviews
      </a>
      <?php if ($googleReviewUrl): ?>
      <a href="<?= e($googleReviewUrl) ?>" target="_blank" rel="noopener" class="rev-write-btn btn-gold">
        <i class="fab fa-google text-xs"></i> Review us on Google
      </a>
      <?php endif; ?>
      <?php if ($tripadvisorUrl): ?>
      <a href="<?= e($tripadvisorUrl) ?>" target="_blank" rel="noopener"
